 ...
 * event.
 */
function OnError(priority = domain_event_handler_priority_enum_1.DomainEventHandlerPriority.MEDIUM, label = foundation_1.UUID.V4().id(), stopPropogationOnError = false) {
    return function (parentCls, funcName, descriptor) {
        ...
        // get the event name.
        const eventName = event_aggregate__type_1.EventAggregate.Error.toString();
        if (func) {
