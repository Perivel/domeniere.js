...
export function OnInternal(priority = DomainEventHandlerPriority.MEDIUM, label = UUID.V4().id(), stopPropogationOnError = false) {
    return function (parentCls, funcName, descriptor) {
        const func = descriptor.value;
        const handlerPriority = Number(priority);
        const eventName = EventAggregate.Internal.toString();
        if (func) {
