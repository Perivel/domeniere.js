...
```
Here, we are defining a an EventStore called `MyEventStore`. We then export it along with our library to be implemented in our infrastructure layer (the framework).

### Implementing the EventStore
We implement our `EventStore` in the infrastructure layer of our application. THat is, in our framework. The defined `EventStore` is then injected into our API during runtime. For more information about the API, see the API section.
...
## Event Handlers
Event Handlers are methods that are automatically executed in response to some event that has been emitted. All event handlers have the following signiture.
```ts
async methodName(event: DomainEvent): Promise<void>
```
The Event Hanlder receives the event as its argument. And it is free to do whatever it needs to do in response to the event being emitted. To tell Domeniere this method is an event listener, we need to decorate it with an event decorator. There are four types of event decorators we can use.
...
```ts
@On(UserCreated)
public async logUserId(event: DomainEvent): Promise<void> {
    const createdUser = (event as UserCreated).user();
...
export class UserCreated extends DomainEvent {
    ...
    // customize the event here.
}
```
Notice here, we are defining a `UserCreated` event. We are also passing in two optional arguments to our event. The first is the timestamp of when the event occurs. By default, this is the current date and time in UTC. The second argument is a unique id for the event instance. When it is set to `undefined`, a random unique ID is generated for us by the `DomainEvent` class.

When defining your custom events, there are three three required methods you must override.
...
The second method we need to override is the `EventClassification()` method. The `EventClassification()` method indicates what group the event belongs to. In most cases, this will be the name of your domain or module.
```ts
export class UserCreated extends DomainEvent {
...
```
Like with the event name, the event classification can be whatever we desire. It is again recommended that the classification be short and descriptive. Unlike the event name, however, several events can fall in the same classification.
...
```
As we have not yet fully define our `CreatedUser` event, we will save the details of this method for later. For now, let's define our `CreatedUser` event.
```ts
export class UserCreated extends DomainEvent {
...
```
Notice here our completed `UserCreated` event class. We simply add one additional property to hold the user object that was created. And, we add a getter method to get the created user.
