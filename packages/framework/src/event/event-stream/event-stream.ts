...
export class EventStream implements EventStreamInterface {
    ...
    public subscribe<T extends DomainEvent>(event: DomainEventClass<T>|EventAggregate, handler: DomainEventHandler<T>, priority: DomainEventHandlerPriority = DomainEventHandlerPriority.MEDIUM, label: string = "", stopPropogationOnError: boolean = false): void {
        ...
        const eventName = Object.values(EventAggregate).includes(event as EventAggregate) ? event.toString() : (event as any).EventName();
