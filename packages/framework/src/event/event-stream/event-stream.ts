...
export class EventStream implements EventStreamInterface {
    ...
    subscribe<T extends DomainEvent>(event: DomainEventClass<T>|EventAggregate, handler: DomainEventHandler<T>, priority: DomainEventHandlerPriority = DomainEventHandlerPriority.MEDIUM, label: string = "", stopPropogationOnError: boolean = false): void {
