...
class Domain {
     ...
     */
    static CreateSubdomain(name, eventStore = new event_module_1.DefaultEventStore()) {
        if (!Domain.ContainsModule(name)) {
            ...
            Domain.Module(frameworkStorageSubmodule).bindInstance(event_module_1.EventStream, new event_module_1.EventStream(eventStore));
        }
        else {
    ...
    static EventStream(context) {
        return Domain.Module(`${context}.__domeniere__`).get(event_module_1.EventStream);
    }
    /**
     * Module()
     *
    ...
    static async PublishEvents(context) {
        await Domain.Module(`${context}.__domeniere__`).get(event_module_1.EventStream).publishEvents();
    }
}
