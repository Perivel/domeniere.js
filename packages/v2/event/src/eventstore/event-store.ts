...
import { EventsBradcastedListenerFn } from "./events-broadcasted-listener.type";
...
export abstract class EventStore {
    ...
    private _onEentsBroadcasted: EventsBradcastedListenerFn|null
    ...
    /**
     * _onEventsBroadcasted()
     * 
     * attaches a handler to be executed when events have been successfully broadcasted. This is for internal use only.
     * To listen for broadcasted events in your application, register a listener to the EventsPublished Internal event.
     * @param handler the handler to execute.
     */

    public _onEventsBroadcasted(handler: EventsBradcastedListenerFn): void {
        this._onEentsBroadcasted = handler;
    }
    ...
    public async publishEvents(): Promise<void> {
        ...
        if (!this._publishQueue.isEmpty()) {
            try {
                ...
                if (!this._broadcastedQueue.isEmpty() && this._onEentsBroadcasted) {
                    ...
                    await this._onEentsBroadcasted(this._broadcastedQueue);
                }
            }
        }
    }
